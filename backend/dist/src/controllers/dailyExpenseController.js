...
export const syncDailyExpenses = async (req, res) => {
    const userId = req.userId;
    if (!userId) {
        return res.status(401).json({ message: "User not authenticated" });
    }
    try {
        ...
        for (const item of aggregated) {
            ...
            const updated = await dailyExpense.findOneAndUpdate({ user: userId, date: dateStr }, { totalExpense: total }, { new: true, upsert: true });
...
export const getDailyExpenseByDate = async (req, res) => {
    const userId = req.userId;
...
//# sourceMappingURL=dailyExpenseController.js.map